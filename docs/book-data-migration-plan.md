...
## 현재 상태와 다음 작업 (2026-09-07 기준)

**알라딘 종료까지 53일.** 되돌릴 수 없는 항목부터 처리합니다.

| 순서 | 할 일 | 성격 | 참조 |
| --- | --- | --- | --- |
| **1** | 표지 원본 4.47GB를 두 번째 장소에 백업 | **비가역.** 현재 맥 한 곳에만 있음 | Phase 0 체크리스트 |
| **2** | D9 결정 — 신규 ISBN 서지·검색 공급처 | **기한.** 미정이면 10/30에 등록 기능 정지 | 6장, 6-d |
| 3 | 카카오 어댑터 구현 (D9가 카카오로 정해질 경우) | 가역 | 6-d |
| 4 | D1 확정 → R2 업로드 → `books.image` 컷오버 | 가역 | Phase 2, 7-c |
| 5 | 신규 도서 표지 상시 파이프라인 | 가역. 없으면 컷오버가 계속 샘 | 7-b |
| 6 | 10/30 직전 표지 델타 재수집 (약 13분) | **기한** | Phase 0 체크리스트 |
| 7 | 알라딘 코드 전면 제거 | 2~6 완료 후 언제든 | Phase 4 |

### 끝난 것

- Phase 0 표지 원본 수집 (56,652건 / 4.47GB) — **보관처 이중화만 남음**
...
상세는 자체 DB 우선으로 전환됐습니다. 검색 승격(또는 병합)은 D9 결정과 함께
판단합니다. 4장을 보세요.
...
- [ ] **매니페스트 + 원본을 두 곳 이상에 보관** ← 남은 유일한 비가역 항목
  ...
  - 방법: **하나의 아카이브로 묶어 Google Drive에 올립니다.** 사용자가 Google
    AI Pro 구독 중이라 2TB 중 4.47GB(0.2%)로 여유가 충분합니다.
  - 56,652개 파일을 그대로 동기화하면 매우 느리고 잘 깨집니다. 반드시 묶으세요.
    JPEG는 이미 압축돼 있어 무압축 `tar`가 빠릅니다.
...
"DB에 없는 신규 도서만 카카오로"는 `resolveBook`(ISBN 단건)에서는 그대로
성립합니다. 그러나 **키워드 검색은 DB에 없는지를 외부에 물어보기 전에는 알 수
없습니다.**

| 시점 | 검색 체인 |
| --- | --- |
| 지금 | 알라딘 우선. 인덱스는 깔렸지만 승격은 안 함 |
| 카카오 도입 직후 | 카카오 우선 — 알라딘 자리를 그대로 대체 |
| 그다음 | 자체 DB + 카카오 **병합**(ISBN 기준 dedupe) |

세 번째는 현재 `runChain`의 "첫 유효 결과에서 멈춤"과 동작이 다릅니다. 필요해질
때 오케스트레이터를 고치면 되며, 미리 만들 필요는 없습니다.
