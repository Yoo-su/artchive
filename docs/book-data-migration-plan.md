...
| **5** | **R2 활성화 → 업로드 → `books.image` 컷오버** ← 진행 중 | 가역 | Phase 2, 7-c |
| 6 | 신규 도서 표지 상시 파이프라인 | 가역. 없으면 컷오버가 계속 샘 | 7-b |
| 7 | 10/30 직전 표지 델타 재수집 (약 13분) | **기한** | Phase 0 체크리스트 |
| 8 | 알라딘 코드 전면 제거 | 2~7 완료 후 언제든 | Phase 4 |

5번의 남은 단계는 **Cloudflare 계정에 결제수단 등록 → R2 활성화 → 버킷 생성 →
`cdn.bookjeok.com` 커스텀 도메인 연결**입니다. 사용자가 대시보드에서 직접
해야 하는 부분이며, 그 뒤 업로드·컷오버는 스크립트로 진행합니다.
...
- [x] **아카이브 생성** (2026-09-07) — `~/bookjeok-covers-20260907.tar` (4.7GB)
