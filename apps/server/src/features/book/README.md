...
- **`book.service.ts`**: 도서 및 판매글 관련 핵심 비즈니스 로직을 수행합니다.
  - `resolveBook`: 판매글 생성 시, 해당 책이 데이터베이스에 없으면 새로 등록하고, 있으면 기존 정보를 반환하여 데이터 중복을 방지합니다. (Request Collapsing 기술을 사용하여 동시 요청 시 외부 API 중복 호출을 방지하고, 불필요한 후속 조회를 제거하여 리소스를 최적화했습니다.)
...
| :---------- | :------------------ | :----------------------------------------------- | :---------------- |
| `POST`      | `/sale`             | 새로운 중고 서적 판매글을 생성합니다. (BookResolvePipe 적용) | ✅ (Access Token) |
...
4.  **`resolveBook`**: `BookService` 내부적으로 `resolveBook` 메서드를 호출합니다.
...
### BookResolvePipe 최적화 사항

`BookResolvePipe`는 리소스 부하를 최소화하기 위해 다음과 같은 최적화 로직이 적용되어 있습니다.

- **Request Collapsing (BookService)**: 짧은 시간 내에 동일한 ISBN에 대한 싱크 요청이 여러 건 들어올 경우, 단 한 번의 외부 API 호출만 수행하고 결과를 공유하도록 설계되었습니다.
- **DB 조회 최적화**: 도서 신규 생성 시, 이미 확보한 데이터를 활용하여 불필요한 후속 `SELECT` 쿼리를 제거했습니다.
