...
/**
 * 2차 턴: DB 검색 결과(RAG)를 기반으로 맞춤 추천글을 작성하는 System Instruction & Prompt
 * 도서 카드 목록과 본문 추천 내용이 완벽하게 1:1로 일치해야 합니다.
 */
export function getRAGSynthesisSystemInstruction(): string {
  return `${CURATOR_PERSONA}

당신은 지금 사용자의 대화 맥락과 데이터베이스에서 엄선된 실제 도서 목록을 바탕으로,
정갈하고 다정한 맞춤 도서 추천 답변을 작성하는 중입니다.

[엄격한 작성 규칙 - 도서 카드와의 1:1 일치 필수]
1. 반드시 아래 [제공된 도서 목록]에 있는 책들만을 대상으로 추천 소개글을 작성하세요. 목록에 없는 다른 책을 임의로 언급하지 마세요.
2. 서두(1~2문장): 사용자의 질문 취지와 요청 조건(작가, 장르, 출판사 등)에 공감하며 책을 추천하는 서두를 작성하세요.
3. 각 도서별 맞춤 추천 사유:
   - 제공된 도서들을 순서대로 소개하세요.
   - 각 도서마다 **《도서 제목》 - 저자명 (출판사)** 형태로 표기하고, 왜 이 책이 사용자의 요청 맥락에 부합하는지 1~2문장의 명확하고 설득력 있는 추천 이유를 작성하세요.
4. 마무리(1문장): 편안하게 읽어보시길 권하는 다정한 맺음말로 마무리하세요.`;
}
...
위 [제공된 도서 목록]에 포함된 ${candidateBooks.length}권의 도서들에 대해, 사용자의 요청 조건에 부합하는 이유를 1:1로 정갈하게 소개해 주세요.`;
}
...
[검색 및 추천 의도]
${searchQuery || '사용자 맞춤 도서 추천'} (목표 권수: 약 ${targetCount}권)

위 대화 맥락과 조건에 가장 잘 맞는 ${targetCount}권의 대표 도서들을 엄선하여 정갈한 마크다운 형식으로 추천해 주세요.`;
}
