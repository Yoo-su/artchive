...
// [bookjeok patch] 원본은 마우스 좌표를 (0, 0)으로 초기화해서, 커서가 캔버스에 들어오기
// 전까지 별들이 좌상단 모서리로 빨려 들어간다. 영향권 밖 좌표를 idle 값으로 사용한다.
const OUT_OF_RANGE = {
...
function GravityStarsBackground({
  ...
  const mouseRef = React.useRef<{ x: number; y: number }>({ ...OUT_OF_RANGE });
  ...
  // [bookjeok patch] 포인터가 빠져나가면 마지막 좌표에 별이 계속 붙어 있지 않도록 초기화
  const handlePointerLeave = React.useCallback(() => {
    mouseRef.current = { ...OUT_OF_RANGE };
  }, []);
