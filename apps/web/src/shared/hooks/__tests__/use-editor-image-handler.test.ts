...
describe("useEditorImageHandler", () => {
  ...
  beforeEach(() => {
    ...
    let objUrlCounter = 0;
    mockCreateObjectURL.mockImplementation(() => `blob:test-url-${++objUrlCounter}`);
  ...
  describe("handleImageAdd", () => {
    ...
    it("이미지 검증 실패 시, toast.error를 호출하고 null을 반환해야 한다", async () => {
      ...
      const { validateImageForUpload } = await import("@/shared/utils/compress-image");
      vi.mocked(validateImageForUpload).mockReturnValueOnce("용량 초과 에러 메시지");
  ...
  describe("uploadImages", () => {
    it("업로드할 이미지가 없어도 content와 삭제된 목록을 정상적으로 반환해야 한다", async () => {
      const { result } = renderHook(() =>
        useEditorImageHandler({ uploadPath: mockUploadPath, initialContent: '<img src="https://old.url" />' })
      );

      let uploadResult: { content: string; deletedImageUrls: string[] } | undefined;
      await act(async () => {
        uploadResult = await result.current.uploadImages("새로운 컨텐츠 (이미지 없음)");
      });
    ...
    it("handleImageAdd를 통해 추가된 이미지가 컨텐츠에 포함될 경우 Vercel Blob에 업로드하고 URL을 교체해야 한다", async () => {
      ...
      expect(uploadResult?.content).toBe(`<p>Test content <img src="https://vercel.blob/test.png" /></p>`);
    ...
    it("업로드 실패 시 예외를 던지고 isUploading 상태가 다시 false가 되어야 한다", async () => {
      ...
      let uploadPromise: Promise<{ content: string; deletedImageUrls: string[] }>;
      act(() => {
