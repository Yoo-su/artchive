...
사용자가 "요즘 기분이 울적한데 마음이 따뜻해지는 소설 읽고 싶어요" 같은 자연어를 입력하면, 65,000권 이상의 도서 DB에서 의미론적 유사도를 기반으로 도서를 찾아 추천합니다.

이 기능은 단순 키워드 매칭이 아니라, 도서 메타데이터(제목·저자·줄거리·장르)를 결합한 텍스트를 Google `gemini-embedding-001` 모델로 768차원 벡터로 변환하여 PostgreSQL `pgvector`에 저장하는 사전 임베딩 작업을 거칩니다. 검색 시점에는 사용자의 질문도 동일한 모델로 임베딩한 뒤, 코사인 유사도 기반 벡터 검색(IVFFlat/HNSW 인덱스)으로 후보 도서를 추출합니다.
...
사용자 세션은 JWT 토큰 인증과 유저 ID별 `sessionStorage` 키 격리를 통해 계정 간 대화 기록이 섞이지 않도록 처리합니다.

---

### AI 도서 핵심 요약

도서 상세 페이지에서 Gemini에 제목·저자·줄거리·출판사 정보를 전달하여, 단순 소개글과 차별화된 분석적 요약을 생성합니다. 응답은 4개의 구조화된 필드로 구성됩니다:
...
프롬프트는 네이버 책 API의 기존 소개글과 중복되지 않도록 설계했습니다. 소개글의 줄거리를 그대로 반복하는 대신, 문학적·역사적 의의나 작가의 독창적 세계관 같은 차별화된 인사이트를 생성하도록 지시합니다.
...
연도별 독서 덱 공유 링크(`/share/deck/[handle]?year=YYYY`)를 통해 카카오톡, 트위터, 슬랙 공유 시 해당 유저의 닉네임과 완독 기록이 반영된 동적 OpenGraph 메타데이터 카드가 생성됩니다.
...
| **Frontend** | Next.js 15 (App Router, RSC, SSG/ISR), Zustand v5, TanStack Query v5, Tailwind CSS v4, Radix UI, Framer Motion, next-intl (ko/en) |
...
| Package | Role |
|---|---|
| `@bookjeok/core` | 데이터 모델 인터페이스, DTO, API 경로 상수 |
| `@bookjeok/api-client` | 공개/인증 Axios 클라이언트 및 엔드포인트 호출 모듈 |
| `@bookjeok/react-query` | TanStack Query 쿼리 키 팩토리 및 데이터 패칭 훅 |
...
│   │   ├── src/app/          # App Router 라우팅 & SEO Metadata
│   │   ├── src/features/     # 도메인 단위 기능 UI & Hooks
│   │   ├── src/views/        # 페이지 뷰 레이어
│   │   └── src/shared/       # 전역 공용 컴포넌트, 스타일, 유틸
│   │
│   ├── server/               # NestJS 11 백엔드 API
│   │   ├── src/features/     # 도메인 모듈 (Search, LLM, User, Review, Sale, Chat)
│   │   └── src/shared/       # 가드, 인터셉터, 데코레이터
│   │
...
- **Rate Limiting** — AI 검색 및 주요 API 엔드포인트에 Throttler 기반 호출 제한
- **이미지 최적화** — Cloudinary 및 Vercel Blob Storage 기반 외부 이미지 도메인 보안 패턴
